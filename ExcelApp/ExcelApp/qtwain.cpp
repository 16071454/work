...
void QTwain::CopyImage(HANDLE hBitmap, TW_IMAGEINFO& info)
{
    ...
    HANDLE hdib=dib->DIBHandle();
    HPALETTE pepe =GetSystemPalette();
    HBITMAP ima =DIBToBitmap(hdib,pepe);

     QPixmap temp = QtWin::fromHBITMAP(ima);
//    if (emitPixmaps())
//    {
...
//    }
 //  else
 //   {
         qWarning("emit  dibAcquired");
        emit dibAcquired(temp);
  //  }
} // !CopyImage()
