...
char libName[512];
	...
	m_hMessageWnd = hWnd;
   // strcpy_s(libName,"TWAINDSM.DLL");
	
    m_hTwainDLL  = LoadLibrary(TEXT("TWAINDSM.DLL"));
	if(m_hTwainDLL != NULL)
					   ...
					   TW_MEMREF pData)
{
    ...
    { QMessageBox::information(NULL,"提示","TWRC_FAILURE");
...
bool CTwain::Acquire(int numImages)
{
    ...
    TW_PENDINGXFERS twPendingXfer = {0};
    int nCounter = 1;
	...
	qWarning("1!");
    //    TW_UINT32    Svalue = 0;
       // bool ret = GetCapability(CAP_DUPLEXENABLED,Svalue);
    bool   ret = SetCapability(CAP_DUPLEXENABLED,false,true);//中间bool单双面，false-单面
        ...
        qWarning("2!");
			if(EnableSource())
            {
                ...
                while (!isTransDone && GetMessage(&msg, NULL, 0, 0))
                {
                    twEvent.TWMessage = MSG_NULL;

                    twRC = CallTwainProc(&m_AppId, &m_Source, DG_CONTROL, DAT_EVENT, MSG_PROCESSEVENT, &twEvent);
                    ...
                    // The message is not from Source
                    ...
                    case MSG_XFERREADY:		// Data transfer is ready
                        ...
                        do
                        {
                            ...
                            //CallTwainProc(&m_AppId, &m_Source, DG_IMAGE, DAT_IMAGEFILEXFER, MSG_GET, NULL);
                              ...
                              //  GetImage(info);
                                ...
                                if(CallTwainProc(&m_AppId,&m_Source,DG_CONTROL,DAT_PENDINGXFERS,MSG_ENDXFER,(TW_MEMREF)&twPendingXfer))
                                {
                        ...
                        break;
                    case MSG_CLOSEDSREQ:	// "Cancel" pressed on the Source UI
                        isTransDone = TRUE;
                        break;
                    default:
                        break;
